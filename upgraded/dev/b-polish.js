...
(()=>{
 ...
 const table=START.table;START.table=(box,message)=>{table(box,message);box.querySelectorAll('.drop').forEach(slot=>{const place=slot.onclick;slot.onclick=event=>{place(event);if(box.querySelectorAll('.filled').length===6&&!box.querySelector('.b-table-result')){const result=document.createElement('img');result.className='b-table-result';result.src=bGameArt(20);result.alt='שולחן שבת מסודר עם חלות, יין, גביע ללא רגל, שני פמוטים גדולים ושניים קטנים';const restart=box.querySelector('.b-restart');if(restart)restart.before(result);else box.append(result);box.classList.add('b-table-completed');result.scrollIntoView({behavior:'smooth',block:'nearest'})}}})};
 const open=openGame;openGame=function(id){gb.classList.remove('b-table-completed');open(id)};
 const needs=START.needs;START.needs=(box,message)=>{needs(box,message);const image=document.createElement('img');image.src=bGameArt(7);image.alt='התינוק נח בעריסה';box.querySelector('.b-needs-picture').replaceChildren(image)};
