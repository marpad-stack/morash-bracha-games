...
(async()=>{const b=await chromium.launch({headless:true,channel:'msedge'}),results=[];
 for(const width of [390,1440]){
  ...
  for(const [mode,count,images] of [['compact',21,40],['large',40,40],['current',1,1]]){
   ...
   assert(await popup.locator('img').evaluateAll(imgs=>imgs.every(i=>i.dataset.branded==='true')));
   await popup.pdf({path:path.join(out,'b-print-'+mode+'-'+width+'.pdf'),preferCSSPageSize:true,printBackground:true,displayHeaderFooter:false});if(mode==='current')await popup.screenshot({path:path.join(out,'b-print-current-'+width+'.png')});results.push({width,mode,pages:count,images,userActivation:true,allImagesReady:true,embeddedBlackLogo:true});console.log(width,mode,count,'pages');
  }await context.close();
