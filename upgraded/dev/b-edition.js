...
(()=>{
 ...
 function pageHTML(index,interactive=true,pointed=PlayReading.enabled){
  ...
  else if(record.type==='art'||record.type==='text'){
   const lines=text(scene.text,scene.n).split('\n'),part=record.type==='art'?lines.slice(0,2):lines.slice(2);
   body=image(scene.art,scene.title)+'<div class="b-illustrated-copy"><div class="b-prose">'+part.map(line=>'<p>'+esc(line)+'</p>').join('')+'</div>'+(record.type==='text'?question():'')+'</div>';
  }
  ...
  return '<article class="b-paper b-paper-'+record.type+'" data-book-page="'+index+'" data-reading-skip><div class="b-paper-content">'+body+'</div>'+(!['cover','again','back'].includes(record.type)?mark:'')+'<span class="b-paper-number">'+(index+1)+'</span></article>';
 }
 function sceneHTML(sceneIndex){
  const scene=E.scenes[sceneIndex],lines=copy(scene.text,scene.n).split('\n');
  return '<article class="b-picture-spread" data-scene="'+sceneIndex+'" data-reading-skip aria-label="'+esc(scene.title)+'"><figure class="b-scene-art"><img class="b-edition-image" src="'+esc(scene.art)+'" alt="'+esc(scene.title)+'" draggable="false"><figcaption class="b-art-unavailable" hidden>האיור לא נטען כרגע. הסיפור מופיע כאן ואפשר להמשיך לקרוא.</figcaption></figure><div class="b-scene-wash" aria-hidden="true"></div><div class="b-scene-copy"><div class="b-prose">'+lines.map(line=>'<p>'+esc(line)+'</p>').join('')+'</div><button class="b-pause-button" data-pause="'+sceneIndex+'"><span aria-hidden="true">✦</span><span><b>רגע יחד</b><small>שאלה או משחק קטן, אם רוצים</small></span><span aria-hidden="true">←</span></button></div>'+mark+'<span class="b-scene-number">'+(sceneIndex+1)+'</span></article>';
 }
 function visiblePages(){
  if(p>0&&p<23){const start=p%2===0?p-1:p;return[start,start+1]}
  if(!media.matches||p===0||p===pages.length-1)return[p];
  const start=p%2===0?p-1:p;return[start,start+1];
 }
 function render(){
  const indices=visiblePages();if(media.matches||p>0&&p<23)p=indices[0];
  const isScene=p>0&&p<23;const sceneIndex=isScene?(p-1)/2:-1;
  spread.innerHTML=isScene?sceneHTML(sceneIndex):indices.map(i=>pageHTML(i)).join('');spread.classList.toggle('b-single-page',indices.length===1);spread.classList.toggle('b-continuous',isScene);
  document.getElementById('prev').disabled=p===0;document.getElementById('next').disabled=indices.at(-1)===pages.length-1;
  document.getElementById('bPageCount').innerHTML=isScene?'רגע '+(sceneIndex+1)+' מתוך '+E.scenes.length:indices.length===2?'עמודים <bdi dir="ltr">'+(indices[0]+1)+'–'+(indices[1]+1)+'</bdi> מתוך '+pages.length:'עמוד '+(p+1)+' מתוך '+pages.length;
 ...
 goPrev=()=>{if(p>0)go(Math.max(0,p-((media.matches||p>1&&p<=23)&&p>1?2:1)))};
 ...
 function openPause(index){const s=E.scenes[index],m=s.moment;
