...
s=replace(s,'<h1>מתחם <em>הקטנטנים</em></h1>','<h1>'+editorial['a']['title']+'</h1>')
s=replace(s,'פינה מתוקה לאחים הגדולים — בזמן שאמא עם התינוק החדש.',editorial['a']['description'])
s=replace(s,'זיכרון · פאזל · צבע לפי מספר','משחקים לשעות פנאי')
s=replace(s,'function openGame(kind){','function openGame(kind){aRoundLevel=aLevel;')
...
# A pending animation must not update a different game after a rapid switch.
for sig in ['function memory(pairs){','function puzzle(mode,n,id){','function matchGame(round,score){','function numberGame(id){']:
    s=replace(s,sig,sig+'aEpoch++;')
head,marker,tail=s.partition('/* games */')
assert marker
...
function aAward(game,score){const id=game+'-'+aRoundLevel;if(!aRecords[id]||aRecords[id]<score){aRecords[id]=score;PlayUI.save('morash-art-records',aRecords);PlayUI.toast('שיא חדש! '+score+' נקודות ✦')}aPaint()}
function aPaint(){document.getElementById('artPoints').textContent=Object.values(aRecords).reduce((a,b)=>a+b,0)+' נקודות בשיאים'}
document.getElementById('artAlbum').onclick=()=>{let d=document.getElementById('artDialog');if(!d){d=document.createElement('dialog');d.id='artDialog';document.body.append(d)}d.innerHTML='<div class="dialog-top"><h2>המדליות שלי</h2><button class="close" id="artClose" aria-label="סגירה">×</button></div><p class="muted">כל שיא נשמר. משחקים שוב ומשפרים את מספר המהלכים!</p><div class="grid">'+['memory','puzzle','match'].map((g,i)=>'<div class="a-medal-card"><div style="font-size:40px">'+['🧠','🧩','🔎'][i]+'</div><h3>'+['זיכרון','פאזל','בלשי החפצים'][i]+'</h3>'+[1,2,3].map(l=>'<p>רמה '+l+' · '+(aRecords[g+'-'+l]??'עדיין לא שיחקנו')+(aRecords[g+'-'+l]?' נקודות':'')+'</p>').join('')+'</div>').join('')+'</div>';d.showModal();document.getElementById('artClose').onclick=()=>d.close()};
// The puzzle already supports tapping; expose the same action to keyboard users.
const aObserver=new MutationObserver(()=>{document.querySelectorAll('.pz:not(.empty)').forEach(t=>{t.role='button';t.tabIndex=0;t.ariaLabel='חלק פאזל';t.onkeydown=e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();t.click()}}})});aObserver.observe(document.getElementById('gBody'),{childList:true,subtree:true});aPaint();
'''
...
'''
exec(compile((DEV/'a-final-build.py').read_text(encoding='utf-8'),str(DEV/'a-final-build.py'),'exec'))
