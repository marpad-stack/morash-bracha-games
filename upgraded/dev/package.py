...
'b':('הסיפור ממשיך בידיים','📖','#e8dff0','40 עמודי הספר, ארבעה איורים מתוקנים, שש פעילויות, המשך קריאה אוטומטי והדפסה מותאמת.','3+ · סיפור, משחק ודמיון'),
...
for game in data:
    ...
    for key,val in game['data'].items():
        if game['id']=='b' and key=='PAGES':
            ...
            body.append('<h3>הספר המקורי · '+str(len(val))+' עמודים</h3><p>המלל בספר מוטמע בתוך תמונות. הנוסח הקיים נשמר עד לקבלת העריכה החדשה. ארבעה איורים תוקנו לפי ההערות, והנוסח שבתוכם נשמר.</p><div class="book-gallery">'+''.join(figs)+'</div>')
