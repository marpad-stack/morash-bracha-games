...
'b':('הסיפור ממשיך בידיים','📖','#e8dff0','סיפור חדש עם מענדי או חני, 28 עמודים, רגעי הקראה, שש פעילויות ושתי אפשרויות הדפסה.','3+ · סיפור, משחק ודמיון'),
...
for directory in ['a-coloring','i-first-steps','b-story-pages','b-story-art']:
...
for game in data:
    ...
    for key,val in game['data'].items():
        if game['id']=='b' and key=='PAGES':
            ...
            body.append('<h3>הספר המקורי · '+str(len(val))+' עמודים</h3><p>ארכיון היסטורי של 40 עמודי המקור בלבד. הנוסח החדש שאושר מופיע בהמשך, בשתי גרסאות: מענדי וחני. נוסח הארכיון אינו הסיפור המוצג במשחק.</p><div class="book-gallery">'+''.join(figs)+'</div>')
...
'b':['שולב הנוסח החדש שאושר, בשתי גרסאות עם מענדי או חני. בכל ספר 28 עמודים, 11 סצנות ורגעי שיחה ומשחק לבחירה. נוספו כפולות במחשב, איורים מותאמים ושתי אפשרויות הדפסה.','זיכרון מתמונות הסיפור בשלוש רמות, הלבשה בגרירה ובלחיצה, חידת אורות ברשתות 2×2, 3×3 ו־4×4, ושדרוג שולחן שבת וחפצי התינוק.','שש פעילויות פעילות; הוסרו ההמתנה בלחיצה והפרצופים. נשמרו כוכבים, המשך קריאה וציור להורדה.','מניעת ספירה כפולה בהתאמות ותיקון פעולות שממשיכות לאחר סגירת משחק.'],
...
for g in data:
    ...
    if g['id']=='b':files+=list((GAMES/'b-story-pages').glob('*'))+list((GAMES/'b-story-art').glob('*'))
