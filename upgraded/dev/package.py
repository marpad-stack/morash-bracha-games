...
'b':('הסיפור ממשיך בידיים','📖','#e8dff0','40 עמודי הספר המקוריים, שמונה פעילויות, סימנייה, כוכבים ואתגר אורות חדש.','3+ · סיפור, משחק ודמיון'),
...
for directory in ['a-coloring','i-first-steps']:
...
''' + extra+'</style><body><header><b>הבאת ברכה ✦</b><span>מסירת משחקים · 15.09.2026</span></header><main>'+body+'</main></body></html>'
...
for game in data:
    ...
    for key,val in game['data'].items():
        if game['id']=='b' and key=='PAGES':
            ...
            body.append('<h3>הספר המקורי · '+str(len(val))+' עמודים</h3><p>המלל בספר מוטמע בתוך תמונות. העמודים המקוריים משולבים כאן ללא תמלול וללא שינוי, כך שגם הניקוד והעימוד נשמרים בדיוק.</p><div class="book-gallery">'+''.join(figs)+'</div>')
...
supp=[]
for game in data:
    src=(GAMES/game['file']).read_text(encoding='utf-8');parser=TextParser();parser.feed(src);strings=parser.lines[:]
...
details={
'a':['שלוש ברירות פתיחה לפי רמה; נוספה אפשרות זיכרון של 4 זוגות לצעירים.','שיאים לפי משחק ורמה, מסך מדליות ושמירה מקומית.','חידוש צבעים וכרטיסי המשחק; הוספת הפעלה במקלדת לחלקי הפאזל.','כל 15 דפי הצביעה והאיורים המקוריים נשמרו.'],
'b':['40 עמודי הספר נשמרו בדיוק, כולל האיורים והטקסט המוטמע בתוכם.','זיכרון בשלוש רמות: 3, 6 ו־8 זוגות; אתגר אורות קשורים לרמות המתקדמות.','כוכבי משחק, שמירת סימנייה וייצוא ציור כתמונה.','מניעת ספירה כפולה בהתאמות ותיקון פעולות שממשיכות לאחר סגירת משחק.'],
