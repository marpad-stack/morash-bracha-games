...
document.getElementById('printBtn').onclick=()=>{
 ...
 async function prepare(auto){
  const token=++revision;button.disabled=true;textVersion.hidden=true;textVersion.onclick=null;doc.body.dataset.printReady='false';doc.body.dataset.textPages='0';status.textContent='טוענים את כל התמונות…';
  ...
  }catch{if(!printWindow.closed&&token===revision)status.textContent='ההדפסה ממתינה: חלק מהאיורים חסרים. בדקו את החיבור או את סינון התמונות, חזרו לספר ונסו שוב. אפשר להכין גרסה עם טקסט במקום האיורים החסרים, או לבחור רק עמוד שנטען במלואו.'}
 }
