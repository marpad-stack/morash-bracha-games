...
(async()=>{
 ...
 for(const width of [320,390,768,1440]){
  ...
  assert.equal(await p.locator('#bBookSpread .b-paper').count(),width>=900?2:1);await p.screenshot({path:path.join(out,'chani-'+width+'.png')});
 }
 for(const id of ['tower','yawn','find']){await p.evaluate(id=>BStory.openFun(id),id);assert.equal((await p.locator('#bBookFun').textContent()).includes('מענדי'),false);await p.keyboard.press('Escape')}
 ok('Chani manuscript, UI, three end activities, four widths, direct URL and separate bookmarks');
 ...
 if(!base.startsWith('file:')){
  ...
  const q=await blocked.newPage();await q.goto(base,{waitUntil:'domcontentloaded'});await q.evaluate(()=>BStory.go(1));await q.locator('.b-art-unavailable:visible').waitFor();await q.evaluate(()=>BStory.go(2));assert((await q.locator('.b-prose').textContent()).length>100);
