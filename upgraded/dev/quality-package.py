...
s=s.replace('</main>','<h2>תוספת לסבב השדרוג — קריאה נגישה</h2><p>הנוסח החדש שאושר מופיע להלן עם ניקוד ובלעדיו, בגרסאות מענדי וחני. כל גרסה מעוצבת בספר בן 28 עמודים, עם איור וטקסט בעמודים נפרדים על רקע משותף ופעילויות משחק בסוף. התמונות המקוריות שלמעלה הן ארכיון בלבד.</p>'+''.join(reading_html)+'</main>')
p.write_text(s,encoding='utf-8')
