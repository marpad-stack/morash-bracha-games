...
(async()=>{
 ...
 const assets=await p.evaluate(async()=>{const unique=[...new Set(Object.values(BStory.editions).flatMap(x=>[...Object.values(x.art),...Object.values(x.spreads)]))];return Promise.all(unique.map(async src=>{const i=new Image();i.src=await MorashBrand.bookPage(src);await i.decode();return{src,width:i.naturalWidth}}))});assert.equal(assets.length,34);assert(assets.every(x=>x.width>=640));
